...
const miniIconSets = {
  ...
  "cute-cat-island-game": {
    play: "./assets/icons/a34f30/play.webp",
    stop: "./assets/icons/a34f30/stop.webp",
    loading: "./assets/icons/a34f30/loading.gif",
    previous: "./assets/icons/a34f30/prev.webp",
    next: "./assets/icons/a34f30/next.webp",
    shuffleActive: "./assets/icons/a34f30/shuffle-active.webp",
    shufflePassive: "./assets/icons/696969/shuffle-passive.webp",
  },
  "seeds-of-sorrow": {
...
const legacySoundtrackHashes = {
  ...
  "cute-cat-island-game-listen": "cute-cat-island-game",
...
function updateMiniProgress(trackElement, audio) {
  ...
  updateMediaSessionPosition(trackElement, audio);
}
...
function setMediaSessionHandler(action, handler) {
  ...
  try {
    ...
    // Some mobile browsers expose Media Session partially and reject unsupported actions.
  }
}

function formatMediaSessionTitle(entry) {
  if (!entry) {
    return "Eskimo";
  }

  return entry.track.detail
    ? `${entry.track.title} (${entry.track.detail})`
    : entry.track.title;
}

function updateMediaSessionMetadata(entry) {
  if (!hasMediaSession() || typeof window.MediaMetadata !== "function" || !entry) {
    return false;
  }

  const trackKey = `${entry.index}:${entry.track.title}:${entry.track.detail || ""}`;

  if (mediaSessionTrackKey === trackKey) {
    return false;
  }

  mediaSessionTrackKey = trackKey;
...
function updateMediaSessionPlaybackState(hasTrack, isPlaying) {
  if (!hasMediaSession()) {
    return;
  }

  navigator.mediaSession.playbackState = hasTrack ? (isPlaying ? "playing" : "paused") : "none";
}

function updateMediaSessionPosition(trackElement, audio, force = false) {
  if (
    !hasMediaSession() ||
  ...
  if (!force && now - lastMediaSessionPositionUpdate < 1000) {
    return;
  }
  ...
  try {
    ...
    // Invalid transient media states should not interrupt site playback.
  }
}

function syncMediaSession(entry, hasTrack, isPlaying) {
  if (!hasMediaSession()) {
    return;
  }

  if (!hasTrack || !entry) {
    mediaSessionTrackKey = "";
    navigator.mediaSession.metadata = null;
    updateMediaSessionPlaybackState(false, false);
    return;
  }

  const didUpdateMetadata = updateMediaSessionMetadata(entry);
  updateMediaSessionPlaybackState(true, isPlaying);
  updateMediaSessionPosition(entry.trackElement, entry.audio, didUpdateMetadata);
}
...
function initMediaSession() {
  ...
  setMediaSessionHandler("play", () => {
    if (activeAudio && activeTrack && activeAudio.paused) {
      toggleTrack(activeTrack, activeAudio, { forcePlay: true, resetNewTrack: false });
    }
  });

  setMediaSessionHandler("pause", () => {
    if (activeAudio && activeTrack && !activeAudio.paused) {
      fadeOutAndPause(activeAudio, activeTrack, false);
    }
  });
...
function setDefaultAudioTrack() {
  ...
  const entry =
    playableEntries.find(
      ({ track, trackElement }) =>
        track.title === "ammonia" || trackElement.dataset.trackSlug === "ammonia",
    ) || playableEntries[0];
  activeAudio = entry.audio;
  activeTrack = entry.trackElement;
  resetTrackProgress(activeTrack, activeAudio);
  createShuffleSequence(entry);
  miniShuffle?.setAttribute("aria-pressed", "true");
  preloadAdjacentTrack(entry);
  syncMiniPlayer();
}
